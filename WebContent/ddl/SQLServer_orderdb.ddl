CREATE DATABASE orders;
go

USE orders;
go

DROP TABLE review;
DROP TABLE shipment;
DROP TABLE productinventory;
DROP TABLE warehouse;
DROP TABLE orderproduct;
DROP TABLE incart;
DROP TABLE product;
DROP TABLE category;
DROP TABLE ordersummary;
DROP TABLE paymentmethod;
DROP TABLE customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Basketball Shoes');
INSERT INTO category(categoryName) VALUES ('Soccer Cleats');
INSERT INTO category(categoryName) VALUES ('Street Shoes');


INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('LeBron 20 EP',1,'Lebrons latest hoop shoe',280.00);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Adidas Harden Vol. 5',1,'Signature shoe with comfortable fit and traction for quick movements',150);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Jordan XXXV',1,'Signature shoe with comfortable fit and traction',280.00);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Under Armour Curry 8',1,'Steph Currys latest signature shoe offering stability and sleek design',160);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Nike Kobe AD Exodus',1,'Kobes best basketball shoe',200);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Reebok Question Mid',1,'Timeless design and superior comfort in Allen Iversons signature sneaker',140);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('New Balance OMN1S',1,'Innovative basketball shoe with a focus on speed',170);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Anta KT5',1,'Klay Thompsons shoe offering lightweight feel and reliable traction',130);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Converse All Star Pro BB',1,'Classic Converse style with modern basketball performance features',110);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Li-Ning Way of Wade 9',1,'Dwyane Wades latest shoe delivering style and performance on the court',160);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Li-Ning Speed VI',1,'Speed-focused basketball shoe with Li-Nings advanced cushioning',130);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Adidas Copa Mundial',2,'Legendary soccer cleat known for its timeless design and premium materials',150);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Nike Mercurial Vapor 14',2,'High-performance soccer cleat designed for speed and agility',250);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Diadora Brasil Italy OG MD PU',2,'Classic soccer cleat with Italian craftsmanship and durable construction',170);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Lotto Solista 100 III Gravity',2,'Innovative soccer cleat with Lottos Gravity sole for maximum traction',160);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Nike Tiempo Legend 9',2,'Iconic soccer cleat with premium leather and modern performance features',230);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Adidas Predator Freak.1',2,'Powerful soccer cleat with Demonskin technology for enhanced control',220);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('New Balance Tek',2,'I have no idea what these are',200);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Adidas Superstar',3,'Iconic street shoe with a timeless shell-toe design and classic style',80);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Nike Air Force 1',3,'Legendary sneaker with Nike Air cushioning known for its versatility and comfort',90);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Vans Old Skool',3,'Classic skate shoe with a side stripe known for its durability and style',60);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Converse Chuck Taylor All Star',3,'Timeless canvas sneaker with Chuck Taylor''s signature design',55);
INSERT INTO product(productName, categoryId, productDesc, productPrice) VALUES ('Puma Cali',3,'Street-ready sneaker with a chunky sole and Puma''s signature branding',70);



INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (1, 1, 5, 280);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (2, 1, 10, 150);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (3, 1, 3, 280);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (4, 1, 2, 160);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (5, 1, 6, 120);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (6, 1, 3, 140);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (7, 1, 1,170);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (8, 1, 0, 130);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (9, 1, 2, 110);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (10, 1, 3, 160);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (11, 1, 12, 130);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (12, 1, 50, 150);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (13, 1, 43, 160);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (14, 1, 12, 140);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (15, 1, 43, 110);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (16, 1, 2, 120);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (17, 1, 13, 130);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (18, 1, 43, 140);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (19, 1, 63, 180);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (20, 1, 63, 150);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (21, 1, 33, 110);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (22, 1, 31, 120);
INSERT INTO productInventory(productId, warehouseId, quantity, price) VALUES (23, 1, 30, 130);


INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , '304Arnold!');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , '304Bobby!');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , '304Candace!');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , '304Darren!');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , '304Beth!');

-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2023-10-15 10:25:55', 91.70)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 1, 1, 18)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 2, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 1, 31);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2023-10-16 18:00:00', 106.75)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 21.35);

-- Order 3 cannot be shipped as do not have enough inventory for item 7
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2023-10-15 3:30:22', 140)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 30);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2023-10-17 05:45:11', 327.85)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 3, 4, 10)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 8, 3, 40)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 23.25)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 28, 2, 21.05)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 29, 4, 14);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (5, '2023-10-15 10:25:55', 277.40)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 4, 21.35)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 19, 2, 81)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 20, 3, 10);

-- New SQL DDL for lab 8
UPDATE product SET productImageURL = 'img/1.jpg' WHERE productId = 1;
UPDATE product SET productImageURL = 'img/2.jpg' WHERE productId = 2;
UPDATE product SET productImageURL = 'img/3.jpg' WHERE productId = 3;
UPDATE product SET productImageURL = 'img/4.jpg' WHERE productId = 4;
UPDATE product SET productImageURL = 'img/5.jpg' WHERE productId = 5;

-- Loads image data for product 1
UPDATE product SET productImage =  WHERE ProductId = 1;
